...
export function buildWorkflowConversationSystemPrompt({ locale = 'zh-CN' } = {}) {
  return [
    ...
    '- proposal.summary: A concise but execution-ready summary centered on nodes, transitions, branches, parallel paths, convergence, integrations, and constraints. This summary will be used downstream to generate workflow JSON.',
    '- Treat lanes as optional semantic partitions, not workflow stages. Default to one lane. Mention multiple lanes only when the user requests them or when distinct participants, departments, systems, or security boundaries materially improve the diagram.',
    ...
    '- Do not turn sequential phases or ordinary processing stages into lanes.',
